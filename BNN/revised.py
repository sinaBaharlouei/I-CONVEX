...
def deepnn(x1, x2, W1_det):
    ...
    W_conv1 = tf.reshape(W1_det, [3, 4, 1, 64])
    #  W_conv1 = weight_variable([3, 4, 1, 64])
    ...
    h2_conv1 = tf.nn.relu(conv2d_1(x2_image, W_conv1) - 2)

    # Pooling layer - downsamples by 2X.
    h1_pool1 = max_pool_5x1(h1_conv1)
    h2_pool1 = max_pool_5x1(h2_conv1)
    ...
    W_conv3 = weight_variable([4, 1, 32, 20])
    #  h1_conv3 = tf.nn.relu(tf.nn.tanh(tf.multiply(tanh_const,conv2d(h1_pool2, W_conv3) + b_conv3)))
    #  h2_conv3 = tf.nn.relu(tf.nn.tanh(tf.multiply(tanh_const,conv2d(h2_pool2, W_conv3) + b_conv3)))
    h1_conv3 = tf.nn.relu(tf.nn.tanh(tf.multiply(tanh_const, conv2d(h1_pool2, W_conv3))))
    h2_conv3 = tf.nn.relu(tf.nn.tanh(tf.multiply(tanh_const, conv2d(h2_pool2, W_conv3))))

    # Third pooling layer.
    #  h1_pool3 = max_pool_2x1(h1_conv3)
    #  h2_pool3 = max_pool_2x1(h2_conv3)
    #  h1_pool3 = h1_conv3
    #  h2_pool3 = h2_conv3
    h1_pool3 = max_pool_2x1(h1_conv3)
    h2_pool3 = max_pool_2x1(h2_conv3)
    ...
    W_fc1 = weight_variable([100, 40])

    h1_pool2_flat = tf.reshape(h1_pool3, [-1, 100])
    h2_pool2_flat = tf.reshape(h2_pool3, [-1, 100])
    ...
    inner_product = tf.reduce_sum(tf.multiply(h1_fc1, h2_fc1) + b_final, 1, keep_dims=True)
...
def main(_):
    ...
    y_conv, W1_temp, W2_temp, W3_temp, h1_temp = deepnn(x1, x2, W1_vals)
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_iter):
            ...
            if i % 100 == 0:
                train_accuracy = accuracy.eval(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})
                print('step %d, training accuracy %g' % (i, train_accuracy))

            train_step.run(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})
        ...
        for i in range(batch_num):
            ...
            test_accuracy += accuracy.eval(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})
            total_FP += false_pos_sum.eval(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})
            total_FN += false_neg_sum.eval(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})

            FPs = false_pos.eval(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})
            FNs = false_neg.eval(feed_dict={x1: batch1[0], x2: batch2[0], W1_vals: W1s, y_: batch1[1]})
